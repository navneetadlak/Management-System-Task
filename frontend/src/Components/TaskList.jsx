...
const TaskList = () => {
    ...
    const fetchTasks = async () => {
      try {
        const response = await axios.get(
          `https://task-management-backend-v053.onrender.com/task?page=${currentPage}`
        );
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(
        `https://task-management-backend-v053.onrender.com/task/delete/${selectedTaskId}`
      );
      toast({
        title: "Success",
  ...
  const handleEdit = async () => {
    try {
      await axios.put(
        `https://task-management-backend-v053.onrender.com/task/update/${selectedEditId}`,
        form
      );
      toast({
        title: "Success",
  ...
  const handleStatusUpdate = async (id) => {
    try {
      await axios.put(
        `https://task-management-backend-v053.onrender.com/task/complete/${id}`
      );
      toast({
        title: "Success",
  ...
  return (
    <Box p={4} w={"80%"} bg="gray.100" borderRadius="md">
